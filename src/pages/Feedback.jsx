...
export default function Feedback() {
  ...
  // load orderId from location.state or localStorage.lastOrder
  useEffect(() => {
  ...
  // fetch order to confirm status is completed
  useEffect(() => {
    if (!orderId) return;
    let cancelled = false;
    const fetchOrder = async () => {
      ...
      try {
        ...
        console.error("Failed to fetch order for feedback:", err);
  ...
  const handleSubmit = async () => {
    ...
    // only allow when order status is completed/delivered
    const st = (order?.status || "").toString().toLowerCase();
    if (!(st === "completed" || st === "delivered")) {
      return setError("You can only give feedback after the order is completed.");
    }

    if (rating < 1) return setError("Please provide a rating (1-5).");

    setSubmitting(true);
    try {
      const payload = { rating, message: message || "", orderId };
      if (user && user.token) {
        ...
        if (!res.ok) {
          const body = await res.json().catch(() => ({}));
          throw new Error(body.message || `Failed to submit feedback (${res.status})`);
        }
      } else {
        // guest fallback: save to local
          ...
          {statusLoading ? (
            ...
            <>
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={handleBrowse} className="bg-white border px-6 py-2 rounded-lg font-semibold hover:bg-gray-50">
                  Browse the Menu
                </button>
              </div>
            </>
